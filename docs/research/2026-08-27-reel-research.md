...
| Wideband (Zayed Haq) | private (no repo/site). Parts: ttyd (MIT, Windows, read-only default), tmux (no Windows), Tailscale Serve (tailnet-only), Kokoro (Apache-2.0, 82M), imessage-exporter (GPL-3, read-only, Mac), MentraOS (MIT; G2 apps run on phone). kb ALREADY has tailnet trust mode designed+implemented (docs/superpowers/specs/2026-08-18-tailnet-trust-mode-design.md; kb.command.ts.net -> deploy/kb_node_proxy.py :4319 -> dashboard :4317; identity via root-owned kb_whois_shim.py -> tailscaled LocalAPI /whois; systemd-only; Funnel forbidden; Windows desktop stays 127.0.0.1:5317 (PM2) because the passkey is enrolled for the localhost origin) | ADAPT kill-switch tile (S), fleet-as-thread-list view (M), Kokoro (S); NO ttyd --writable, NO T3 by phone; Windows tailnet mode = L security design |
...
Rev 2.2: 5317 = live Windows desktop listener (PM2), 4319 = VM proxy front (two deployments, not old/new); Windows tailnet mode = L security work + passkey re-enrollment (RP-ID change); hang phone tiles on the VM path.
